...
| 이미 떠 있는 프로세스 위에 `adb install -r`을 반복하면 접근성 서비스가 조용히 먹통이 될 수 있음 | 설정(`enabled_accessibility_services`)엔 등록돼 있고 `dumpsys accessibility`도 바인딩된 것처럼 보이는데, 실제로는 위젯이 전혀 안 뜨고 로그도 전무한 상태가 관찰됨. logcat에 `Package [...] reported as REPLACED, but missing application info. Assuming REMOVED` 라인이 동반됨 — 살아있는 프로세스 위로 코드가 갱신되면서 그 프로세스가 확인 안 된 상태로 남는 것으로 추정. 상세는 8번(2026-08-13, 마지막 항목) 참고 | `adb install -r`로 안 되면 `adb uninstall` 후 완전 재설치. 재현 조건은 명확히 못 밝혔으니, 코드는 멀쩡한데 뭔가 조용히 안 될 때 후보로 의심할 것 |
